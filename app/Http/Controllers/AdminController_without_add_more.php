...
class AdminController extends Controller
{
    ...
    {
    	$data['schedule_list'] = Schedule::where('is_deleted',NULL)->orderBy('created_at','DESC')->get();
    ...
    public function editsSchedule($id=null)
    {
    	$data['schedule_list'] = Schedule::where('is_deleted',NULL)->get();
		$data['id'] = $id;
		$data['timesheetDetail'] = Schedule::where('id',$id)->first();
		$data['ClientList'] = Client::where('is_deleted',NULL)->get();
        $data['StaffList'] = Staff::where('is_deleted',NULL)->get();
        return view('admin/add-schedule',$data);
    }
    ...
    public function submitaddSchedule(Request $request)
    {
		...
		if($message == ""){
			...
			$record = Schedule::where('staff_id',$request->staff_id)->where('date_in',$request->date_in)->where('time_in',$request->time_in)->first();
			
			if(isset($request->id) && !empty($request->id)){
				$record = Schedule::where('staff_id',$request->staff_id)->where('date_in',$request->date_in)->where('id',$request->id)->first();
			...
			if(empty($record)){
				if(!isset($request->id) && empty($request->id)){
					...
					$saveData['admin_notes'] = $request->admin_notes;
